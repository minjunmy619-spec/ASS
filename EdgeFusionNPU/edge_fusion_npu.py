...
class EdgeFusionBlock(nn.Module):
    ...
    def __init__(self, channels: int, *, time_kernel: int, freq_kernel: int, memory_mode: str = "conv"):
        super().__init__()
        if time_kernel < 2:
...
class EdgeFusionNPU(nn.Module):
    ...
    Input:
      x: [B, 2*n_chan, F, 1] packed real/imag STFT frame.

    State:
      one packed 4D cache tensor [B, num_blocks*hidden_channels, F, time_kernel-1].

    Output:
      mask: [B, n_src*n_chan, F, 1] real gain masks. The host runtime applies
    ...
    def forward(self, x: torch.Tensor, state: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        if not torch.onnx.is_in_onnx_export():
            ...
            if x.shape[3] != 1:
                raise ValueError("EdgeFusionNPU exports a single streaming frame per call.")
...
class EdgeFusionNPUExportWrapper(nn.Module):
    """Flatten tuple state I/O for torch.onnx.export."""
