...
class DolphinSFCSeparator(nn.Module):
    """
    ...
    def _build_band_spec(
        ...
        try:
            ...
            if exc.name != "librosa":
                raise
            # Keep structure validation runnable in lean CPU environments.
            return BandSpec2d(n_freq=n_freq, n_bands=n_bands, n_fft=None, sample_rate=None, band_config="linear")
...
def apply_source_gain_mask_4d(x: torch.Tensor, mask_logits: torch.Tensor, n_src: int, n_chan: int) -> torch.Tensor:
    ...
    gains = torch.sigmoid(mask_logits).repeat_interleave(2, dim=1)
    return x_rep * gains


class DolphinSFCStreamingExportWrapper(nn.Module):
    def __init__(self, core: DolphinSFCSeparator, batch_size: int = 1, dtype: torch.dtype = torch.float32):
        super().__init__()
